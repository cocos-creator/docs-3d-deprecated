...
Cocos Creator 3D从 v1.0.3 版本开始正式支持将游戏发布到百度小游戏。
...
我们建议用户只保存脚本文件在小游戏包内，其他资源都从远程服务器下载。Cocos Creator 3D已经帮用户做好了远程资源的下载、缓存和版本管理。具体的实现逻辑和微信小游戏类似，具体可参考 [微信小游戏资源管理](./publish-wechatgame.md#小游戏环境的资源管理)。
...
目前 Cocos Creator 3D 对百度小游戏的适配工作还未完全结束，暂时还不支持以下组件：

- VideoPlayer
- WebView

除此之外分包功能也暂时不可用，用户如果有需要，目前可以先自己直接调用百度的 API 来使用。
