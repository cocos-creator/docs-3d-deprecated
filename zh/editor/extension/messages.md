...
之后，每当场景准备就绪后，广播 scene:ready，就会触发 "hello-world" 扩展插件里的 "initData" 方法。
...
加上 packageName 是为了防止命名出现冲突。在自己的扩展插件里，监听的时候就需要直接标明监听自谁的哪个动作。

这样在 package.json 里更加直观的了解插件的处理流程。

## 编辑器以及扩展插件开放的消息列表

编辑器内的功能插件对外开放的消息列表，可以通过 "开发者" - "消息列表" 面板查看。详细定义规则请参考 [contributions.messages](./contributions-messages.md)
...
send 方法只发送消息，并不会等待返回，如果不需要放会，请使用这个方法。

request 方法返回一个 promise 对象，这个 promise 会接收消息处理后返回的的数据。

broadcast 方法只发送，并且发送给所有监听的功能扩展。