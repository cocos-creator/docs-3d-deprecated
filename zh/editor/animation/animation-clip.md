...
组件包含的属性前会加上组件的名字，比如 `Sprite.spriteFrame`。
属性轨道上对应的蓝色棱形就是关键帧。
...
在动画编辑器的节点项位置右键，选择 `复制节点数据`，在需要粘贴的节点项处右键选择 `粘贴节点数据`即可。该复制粘贴支持快捷键、及其跨编辑器的数据粘贴。
...
右键点击属性列表项，选择移除属性轨道，或者选中当前属性轨道按下 `Delete / Cmd + Backspace` 快捷键移除。
