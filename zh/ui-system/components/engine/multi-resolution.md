...
Cocos Creator 3D 仍然提供一套资源适配多种分辨率屏幕的方案。简单概括来说，可以通过以下几个部分完成多分辨率适配解决方案：
...
当前的设计模式并不允许多分辨率共存，所以在同一个项目里的多个 Canvas 的设计分辨率仍然采用同一套设计分辨率以及适配方案，开发者可以通过 **项目 -> 项目设置 -> 通用设置** 页面配置。
