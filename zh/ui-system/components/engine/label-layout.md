...
**Overflow（排版模式）** 属性，决定了文字内容增加时，如何在约束框的范围内排布。共有 **NONE**、**CLAMP**、**SHRINK**、**RESIZE_HEIGHT** 四种模式，而只有在 ==后三种模式== 下才能通过编辑器左上角的 **矩形变换工具** （也可以是按键盘按键 **T**）或者修改 **属性检查器** 中的 **Size** 大小或者添加 **Widget 组件** 来调整约束框的大小。NONE 模式会自动根据文字尺寸、行高等固定约束框尺寸。
