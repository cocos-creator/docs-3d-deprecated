...
__ProgressBar__ is usually used to show the progress of a certain operation in the game. Add the __ProgressBar__ component to a node and associate a __Bar Sprite__ to this component. Then the __Bar Sprite__ can be controlled to show progress in the scene.
...
Bar Sprite could be its own node, child node or any node that comes with the __Sprite__ component. Also, Bar Sprite can freely choose the `Simple`, `Sliced` or `Filled` render modes.

The __Type__ of __BarSprite__ also needs to be set to __FILLED__, otherwise a warning will appear when the mode of the progressbar has already selected __FILLED__.
